// src/components/calculators/unitConverters/WeightConverter.tsx
...
const WeightConverter: React.FC = () => {
  ...
  const [outputUnit, setOutputUnit] = useState('t');
  ...
  const handleConversion = () => {
    ...
    if (!isNaN(numInputValue)) {
      const valueInKg = numInputValue * conversionFactors[inputUnit];
      const convertedValue = valueInKg / conversionFactors[outputUnit];
  ...
  return (
    <div>
      <h3 className="text-lg font-semibold mb-2">{t('weight_converter_title')}</h3>
