...
import {
  subtractFractions, formatFraction,
  isValidFraction, fractionToDecimal, subtractionSteps,
  type Fraction, type FractionStep
} from '@/utils/math/fractions';

const FractionSubtraction: React.FC = () => {
  ...
  const result = bothValid ? subtractFractions(a, b) : null;
  const steps: FractionStep[] = bothValid ? subtractionSteps(a, b) : [];

  const stepLabels: Record<string, string> = {
    find_lcd: t('fractions_step_find_lcd'),
    convert_fractions: t('fractions_step_convert'),
    subtract_numerators: t('fractions_step_subtract_numerators'),
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <CalculatorInput id="sub-num1" label={t('fractions_numerator_1')} value={num1} onChange={setNum1} placeholder="0" step="1" />
        <CalculatorInput id="sub-den1" label={t('fractions_denominator_1')} value={den1} onChange={setDen1} placeholder="0" step="1" error={errors.den1} />
      </div>
      <div className="text-center text-xl font-bold text-muted-foreground">-</div>
      <div className="grid grid-cols-2 gap-4">
        <CalculatorInput id="sub-num2" label={t('fractions_numerator_2')} value={num2} onChange={setNum2} placeholder="0" step="1" />
        <CalculatorInput id="sub-den2" label={t('fractions_denominator_2')} value={den2} onChange={setDen2} placeholder="0" step="1" error={errors.den2} />
      </div>
...
export default FractionSubtraction;
